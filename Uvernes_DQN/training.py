...
if not os.path.isdir(POLICY_NET__DIR_PATH):
    os.mkdir(POLICY_NET__DIR_PATH)
...
# Observation space was a dict. Must be flattened for training
...
def plot_returns(show_result=False):
    plt.figure(1)
    durations_t = torch.tensor(returns, dtype=torch.float)
    if show_result:
        plt.title('Result')
    else:
        plt.clf()
    ...
    # Take 100 episode return averages and plot them too
...
# We save the policy network 
# (no need to save the target one. Could just reinit to the policy one if we wanted to continue training)
model_path = os.path.join(POLICY_NET__DIR_PATH, POLICY_NET_FILENAME)
torch.save(policy_net.state_dict(), model_path)

policy_net_2 = DQN(n_observations, n_actions)
policy_net_2.load_state_dict(torch.load(os.path.join(POLICY_NET__DIR_PATH, POLICY_NET_FILENAME)))
